...
# central_app.py, que é quem de fato lê essa tabela — mantido em sincronia
# manualmente com MODULO_SLUG_PARA_LABEL de lá (mesma decisão de duplicar
# User/Permissao em vez de um módulo de modelos compartilhado, ver plano da
# Fase 5: são 8 strings estáveis, risco de deriva baixo).
MODULOS_VALIDOS = [
    'pedidos', 'fretes', 'armazenagem', 'estoque',
    'cap_operacional', 'recebimentos', 'fat_dist', 'fat_arm',
]
...
# RelatorioGerado (schema central) mora em central_app.py — Fase 5 moveu o
# domínio Central para o processo/container próprio (ver docker-compose.prod.yml).
...
def deletar_usuario(user_id):
    ...
    # RelatorioGerado agora vive no processo/schema da Central (Fase 5) — sem
    # modelo ORM aqui para ele. Mesmo Postgres, então SQL bruto ainda alcança
    # o schema central (nenhuma separação de privilégio a nível de banco
    # ainda, ver plano da Fase 5); mantém o comportamento de nullify exatamente
    # como antes, só sem o import cruzado do modelo de outro processo.
    db.session.execute(
        text('UPDATE central.relatorios_gerados SET usuario_id = NULL WHERE usuario_id = :uid'),
        {'uid': user.id}
    )
...
def atlas_chat():
    ...
    # confiar num usuario_id que este processo já validou.
    token_bruto = request.cookies.get(app.config['JWT_ACCESS_COOKIE_NAME'])
    ...
    try:
        ...
        # Ao contrário das demais tools (executadas pelo frontend), get_dashboard
        # é resolvida aqui, via central_client — nenhum código de tool lê dados
        # da Central diretamente. Fase 5: Central é um processo/container
        # separado (ver central_app.py) — esta é uma chamada HTTP real, o
        # navegador nunca teve nem terá acesso à API interna da Central.
        def _resolver_get_dashboard(args: dict) -> dict:
            modulo = (args or {}).get('modulo') or None
            try:
                return central_client.obter_dashboard(token_bruto, modulo)
